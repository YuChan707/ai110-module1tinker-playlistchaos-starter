...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    total = len(all_songs) # take all songs, and not only hype
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        total_energy = sum(song.get("energy", 0) for song in all_songs) # change the hhype to all_songs
...
def search_songs(
    ...
    for song in songs:
        ...
        if value and q in value: #update the search bar
